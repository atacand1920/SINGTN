...
- (void)startVideoCompositionRequest:(AVAsynchronousVideoCompositionRequest *)request {
    ...
    if (request.sourceTrackIDs.count == 2) {
        ...
        [self renderFrontBuffer:front backBuffer:back toBuffer:destination who : 2];
...
- (void)renderFrontBuffer:(CVPixelBufferRef)front backBuffer:(CVPixelBufferRef)back toBuffer:(CVPixelBufferRef)destination who:(NSInteger) a {
    ...
    CGRect frame = CGRectMake(0, 0, width / 2, height);
    CGContextRef gc = CGBitmapContextCreate(CVPixelBufferGetBaseAddress(destination), width , height, 8, CVPixelBufferGetBytesPerRow(destination), CGImageGetColorSpace(backImage), CGImageGetBitmapInfo(backImage));
        ...
        frame = CGRectMake(width / 2, 0, width / 2, height);
        
     gc = CGBitmapContextCreate(CVPixelBufferGetBaseAddress(destination), width , height, 8, CVPixelBufferGetBytesPerRow(destination), CGImageGetColorSpace(frontImage), CGImageGetBitmapInfo(frontImage));
