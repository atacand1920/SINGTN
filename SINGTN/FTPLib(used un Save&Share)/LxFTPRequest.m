...
- (instancetype)initPrivate {
    if (self = [super init]) {
        ...
        self.successAction = ^(NSObject* result) {};
    ...
    switch (type) {
        ...
        case kCFStreamEventHasBytesAvailable: {
            ...
            if (bytesRead > 0) {
                ...
                request.successAction([NSArray arrayWithArray:resourceArray]);
                [request stop];
            } else {
                CFStreamError error = CFReadStreamGetError(stream);
                request.failAction((CFStreamErrorDomain)error.domain, (NSInteger)error.error, [request errorMessageOfCode:error.error]);
    ...
    switch (type) {
        ...
        case kCFStreamEventOpenCompleted: {
            ...
            if (request.finishedSize >= request.totalSize) {
                request.successAction(request.localFileURL.absoluteString.stringByDeletingScheme);
                [request stop];
            }
        } break;
        case kCFStreamEventHasBytesAvailable: {
    ...
    switch (type) {
        ...
        case kCFStreamEventCanAcceptBytes: {
            ...
            if (bytesRead > 0) {
                ...
                request.successAction( request.serverURL.absoluteString);
                [request stop];
            } else {
                CFStreamError error = CFWriteStreamGetError(stream);
                request.failAction((CFStreamErrorDomain)error.domain, (NSInteger)error.error, [request errorMessageOfCode:error.error]);
...
#pragma clang diagnostic pop
    ...
    if (destroyResourceSuccess) {
        self.successAction( self.serverURL.absoluteString);
        return YES;
    } else {
