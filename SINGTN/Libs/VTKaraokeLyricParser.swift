...
extension VTKaraokeLyricParser {
    func lyricFromLocalPathFileName(lrcFileName:String) -> VTKaraokeLyric? {
        if let url = URL(string: lrcFileName) {
            do {
                let contents = try String(contentsOf: url, encoding: String.Encoding.utf8)
                print(contents)
                return self.lyricFromLRCString(lrcStr: contents)
        ...
        for row in lyricRows {
            ...
            if row.hasPrefix("[") {
                ...
                if row.hasPrefix("[ti:") || row.hasPrefix("[ar:") || row.hasPrefix("[al:") || row.hasPrefix("[by:") {
                    ...
                    let text = row[row.index(row.startIndex, offsetBy: 5)...row.index(row.endIndex, offsetBy: -2)]
                   
                    switch row[row.index(row.startIndex, offsetBy: 1)...row.index(row.startIndex, offsetBy: 2)] {
                        case "ti":
                            title       = String(text)
                        case "ar":
                            artist      = String(text)
