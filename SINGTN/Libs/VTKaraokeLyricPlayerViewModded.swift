//
//  VTKaraokeLyricPlayerViewModded.swift
//  SINGTN
//
...
@objc protocol VTLyricPlayerViewModdedDataSource:class {
    
    func timesForLyricPlayerView(playerView: VTKaraokeLyricPlayerViewModded) -> Array<CGFloat>
    func lyricPlayerView(playerView: VTKaraokeLyricPlayerViewModded, atIndex:NSInteger) -> VTKaraokeLyricTextView
    
    @objc optional func lengthOfLyricPlayerView(playerView: VTKaraokeLyricPlayerViewModded) -> CFTimeInterval
    
    func lyricPlayerView(playerView: VTKaraokeLyricPlayerViewModded, allowLyricAnimationAtIndex: NSInteger) -> Bool
}

@objc protocol VTLyricPlayerViewModdedDelegate:class {
    @objc optional func lyricPlayerViewDidStart(playerView: VTKaraokeLyricPlayerViewModded)
    @objc optional func lyricPlayerViewDidStop(playerView: VTKaraokeLyricPlayerViewModded)
}

class VTKaraokeLyricPlayerViewModded: UIView {
    
    weak var dataSource:VTLyricPlayerViewModdedDataSource?
    weak var delegate:VTLyricPlayerViewModdedDelegate?
    ...
    private var lyricTop:VTKaraokeLyricTextView!
   // private var lyricBottom:VTKaraokeLyricLabel!
    ...
    func setupLabels() {
        lyricTop        = VTKaraokeLyricTextView()
    ...
    private func showNextLabel() {
        ...
        var lyricLabel:VTKaraokeLyricTextView!
    ...
    func getColoredText(textV: UITextView, text: String) -> NSMutableAttributedString {
        var attributedString = NSMutableAttributedString()
        let words = textV.text.components(separatedBy: " ")
        for word: String in words {
            if word.hasPrefix("@") || word.hasPrefix("#") {
                let aSize = UIFont.systemFont(ofSize: 15)
                    attributedString.append(NSAttributedString(string: "\(word) ", attributes: [NSAttributedStringKey.font: aSize, NSAttributedStringKey.foregroundColor: UIColor.red]))
                
            } else {
                let aSize = UIFont.systemFont(ofSize: 15)
                    attributedString.append(NSAttributedString(string: "\(word) ", attributes: [NSAttributedStringKey.font: aSize, NSAttributedStringKey.foregroundColor: UIColor.blue]))
        ...
        if attributedString.string.hasSuffix(" ") {
            ...
            lastCharRange?.length = attributedString.length - 1
            if let aRange = lastCharRange {
                attributedString = NSMutableAttributedString(attributedString: attributedString.attributedSubstring(from: aRange))
            }
        }
