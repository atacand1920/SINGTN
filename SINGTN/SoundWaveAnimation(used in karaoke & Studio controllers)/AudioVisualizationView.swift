...
var AudioRecord = false
    ...
    private var currentMeteringLevelsArrayRecorder : [Float] {
        if !self.meteringLevelsClusteredArrayRecorder.isEmpty {
            return meteringLevelsClusteredArrayRecorder
        }
        return meteringLevelsArrayRecorder
    }
    ...
    public var meteringLevelsRecorder: [Float]? {
        didSet {
            if let meteringLevels = self.meteringLevelsRecorder {
                self.meteringLevelsClusteredArrayRecorder = meteringLevels
                self.currentGradientPercentage = 0.0
                _ = self.scaleSoundDataToFitScreenRecorder()
            }
        }
    }
    ...
    static var audioVisualizationDefaultGradientStartColorRecorder: UIColor {
        return UIColor(red: 255.0 / 255.0, green: 147.0 / 255.0, blue: 0 / 255.0, alpha: 1.0)
    }
    static var audioVisualizationDefaultGradientEndColorRecorder: UIColor {
        return UIColor(red: 255.0 / 255.0, green: 255.0 / 255.0, blue: 138.0 / 255.0, alpha: 1.0)
    }
    ...
    @IBInspectable public var gradientStartColorRecorder:UIColor =
        AudioVisualizationView.audioVisualizationDefaultGradientStartColorRecorder {
        didSet {
            self.setNeedsDisplay()
        }
    }
    @IBInspectable public var gradientEndColorRecorder:UIColor =
        AudioVisualizationView.audioVisualizationDefaultGradientEndColorRecorder {
        didSet {
            self.setNeedsDisplay()
        }
    }
	override public func draw(_ rect: CGRect) {
		...
		if let context = UIGraphicsGetCurrentContext() {
            ...
            if let context = UIGraphicsGetCurrentContext() {
                self.drawLevelBarsMaskAndGradientRecorder(inContext: context)
            }
        }
	}
    private func drawLevelBarsMaskAndGradientRecorder(inContext context: CGContext) {
        if self.currentMeteringLevelsArrayRecorder.isEmpty  {
            return
        }
        ...
        self.drawMeteringLevelBarsRecorder(inContext: maskContext!)
        ...
        self.drawGradientRecorder(inContext: context)
        
        context.restoreGState()
        
        
    }
    private func drawMeteringLevelBarsRecorder(inContext context: CGContext) {
        let offset = max(self.currentMeteringLevelsArrayRecorder.count - self.maximumNumberBars, 0)
        
        for index in offset..<self.currentMeteringLevelsArrayRecorder.count {
            self.drawBarRecorder(index - offset, meteringLevelIndex: index, isUpperBar: true, context: context)
            self.drawBarRecorder(index - offset, meteringLevelIndex: index, isUpperBar: false, context: context)
        }
    }
    private func drawBarRecorder(_ barIndex: Int, meteringLevelIndex: Int, isUpperBar: Bool, context: CGContext) {
        context.saveGState()
        
        var barPath: UIBezierPath!
        
        let xPointForMeteringLevel = self.xPointForMeteringLevel(barIndex)
        let heightForMeteringLevel = self.heightForMeteringLevel(self.currentMeteringLevelsArrayRecorder[meteringLevelIndex])
	...
	public func reset() {
        ...
        self.meteringLevelsClusteredArrayRecorder.removeAll()
		self.meteringLevelsArray.removeAll()
        self.meteringLevelsArrayRecorder.removeAll()
		self.setNeedsDisplay()
	}
    private func drawGradientRecorder(inContext context: CGContext) {
        if self.currentMeteringLevelsArrayRecorder.isEmpty {
            return
        }
        ...
        let colors = [self.gradientStartColorRecorder.cgColor, self.gradientEndColorRecorder.cgColor]
        ...
        if self.currentGradientPercentage != nil {
            self.drawPlainBackgroundRecorder(inContext: context, fillFromXCoordinate: endPoint.x)
        }
    }
    private func drawPlainBackgroundRecorder(inContext context: CGContext, fillFromXCoordinate xCoordinate: CGFloat) {
        context.saveGState()
        ...
        self.gradientStartColorRecorder.setFill()
        squarePath.fill()
	...
	public func addMeteringLevel(_ meteringLevel: Float) {
        ...
        self.AudioRecord = false
		self.setNeedsDisplay()
	}
    public func addMeteringLevelRecords(_ meteringLevel: Float) {
        guard self.audioVisualizationMode == .write else {
            fatalError("trying to populate audio visualization view in read mode")
            
        }
        self.meteringLevelsArrayRecorder.append(meteringLevel)
        self.AudioRecord = true
        self.setNeedsDisplay()
    }
    ...
    public func scaleSoundDataToFitScreenRecorder() -> [Float] {
        if self.meteringLevelsArrayRecorder.isEmpty {
            return []
        }
        self.meteringLevelsClusteredArrayRecorder.removeAll()
        var lastPosition: Int = 0
        ...
        for index in 0..<self.maximumNumberBars {
            let position: Float = Float(index) / Float(self.maximumNumberBars) * Float(self.meteringLevelsArrayRecorder.count)
            var h: Float = 0.0
            
            if self.maximumNumberBars > self.meteringLevelsArrayRecorder.count && floor(position) != position {
                let low: Int = Int(floor(position))
                let high: Int = Int(ceil(position))
                
                if high < self.meteringLevelsArrayRecorder.count {
                    h = self.meteringLevelsArrayRecorder[low] + ((position - Float(low)) * (self.meteringLevelsArrayRecorder[high] - self.meteringLevelsArrayRecorder[low]))
                } else {
                    h = self.meteringLevelsArrayRecorder[low]
                }
            } else {
                for nestedIndex in lastPosition...Int(position) {
                    h += self.meteringLevelsArrayRecorder[nestedIndex]
                }
                let stepsNumber = Int(1 + position - Float(lastPosition))
                h = h / Float(stepsNumber)
            }
            ...
            lastPosition = Int(position)
            self.meteringLevelsClusteredArrayRecorder.append(h)
        }
        self.setNeedsDisplay()
        return self.meteringLevelsClusteredArrayRecorder
    }
